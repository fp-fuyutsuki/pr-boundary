...
function parseArgs(argv) {
    const result = { format: "text", help: false };
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.UnsupportedGitStatus = void 0;
exports.parseGitDiff = parseGitDiff;
exports.resolveCommit = resolveCommit;
exports.loadPolicyFromGit = loadPolicyFromGit;
...
class UnsupportedGitStatus extends Error {
}
exports.UnsupportedGitStatus = UnsupportedGitStatus;
...
function parseGitDiff(output) {
    ...
    for (let index = 0; index < tokens.length;) {
        ...
        if (status === "A" || status === "M" || status === "D") {
            ...
            if (!path)
                throw new UnsupportedGitStatus("invalid git path record");
            changes.push({ status: status === "A" ? "added" : status === "M" ? "modified" : "removed", filename: (0, paths_1.normalizeGitPath)(path) });
        }
        else if (status.startsWith("R")) {
            const previous = tokens[index++];
            const next = tokens[index++];
            if (!previous || !next)
                throw new UnsupportedGitStatus("invalid git rename record");
            changes.push({ status: "renamed", previousFilename: (0, paths_1.normalizeGitPath)(previous), filename: (0, paths_1.normalizeGitPath)(next) });
        }
        else {
...
async function loadPolicyFromGit(baseSha) {
...
const git_1 = __nccwpck_require__(6094);
const args_1 = __nccwpck_require__(7593);
const output_1 = __nccwpck_require__(6657);
exports.HELP = "Usage: pr-boundary check --base <ref> --head <ref> [--profile <name>] [--format text|json]";
async function runCli(argv) {
    const args = (0, args_1.parseArgs)(argv);
    if (args.help) {
        process.stdout.write(exports.HELP + "\n");
        return 0;
    }
    if (args.command !== "check" || !args.base || !args.head) {
        process.stderr.write(exports.HELP + "\n");
        return 1;
    }
    let result;
    let baseSha = "";
    let headSha = "";
    try {
        baseSha = await (0, git_1.resolveCommit)(args.base);
        headSha = await (0, git_1.resolveCommit)(args.head);
    }
    catch {
    ...
    let policyText;
    try {
        policyText = await (0, git_1.loadPolicyFromGit)(baseSha);
    }
    catch {
        result = { verdict: "REVIEW_REQUIRED", reasonCodes: ["POLICY_UNAVAILABLE"], findings: [] };
        process.stdout.write(args.format === "json" ? (0, output_1.renderJson)(result, baseSha, headSha) : (0, output_1.renderText)(result));
        return 2;
    }
    let policy;
    try {
        policy = (0, core_1.parsePolicy)(policyText);
    }
    catch {
    ...
    let changes;
    try {
        changes = await (0, git_1.getChangedFiles)(baseSha, headSha);
    }
    catch (error) {
        if (error instanceof git_1.UnsupportedGitStatus) {
    ...
    result = (0, core_1.evaluateChanges)(policy, (0, core_1.selectProfile)(policy, [], args.profile), changes);
    process.stdout.write(args.format === "json" ? (0, output_1.renderJson)(result, baseSha, headSha) : (0, output_1.renderText)(result));
...
function renderText(result) {
    ...
    for (const finding of result.findings)
        lines.push(safeText(finding.path) + " [" + finding.reasons.join(", ") + "]");
    return lines.join("\n") + "\n";
}
function renderJson(result, baseSha, headSha) {
...
const glob_1 = __nccwpck_require__(6085);
const paths_1 = __nccwpck_require__(6107);
const reasons_1 = __nccwpck_require__(9186);
function evaluateChanges(policy, selection, changes) {
    ...
    const allow = profile ? (0, glob_1.compileGlobs)(profile.allow) : [];
    const protectedPatterns = (0, glob_1.compileGlobs)(policy.protected);
    const findings = [];
    const globalReasons = selection.reason ? [selection.reason] : [];
    for (const change of changes) {
        const paths = change.status === "renamed" ? [change.previousFilename, change.filename] : [change.filename];
        for (const path of paths) {
            if (path === undefined || (0, paths_1.validateRepositoryPath)(path)) {
            ...
            if (protectedPath && (!profile || !profile.allowProtected))
                reasons.push("PROTECTED_PATH");
            if (reasons.length > 0)
                findings.push({ path, reasons: (0, reasons_1.sortReasons)(reasons) });
        }
    }
    const reasonCodes = (0, reasons_1.sortReasons)([...globalReasons, ...findings.flatMap((finding) => finding.reasons)]);
...
function compileSegment(segment) {
    ...
    if (unsupported.test(segment))
        throw new Error("unsupported glob syntax");
    if (segment.includes("**"))
        throw new Error("unsupported glob syntax");
    let source = "";
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
__exportStar(__nccwpck_require__(4146), exports);
__exportStar(__nccwpck_require__(2721), exports);
__exportStar(__nccwpck_require__(9905), exports);
__exportStar(__nccwpck_require__(3540), exports);
...
exports.normalizeGitPath = normalizeGitPath;
...
function parsePolicy(text) {
    ...
    const root = record((0, yaml_1.parse)(text));
    ...
    if (!rawProfiles || Object.keys(rawProfiles).length === 0 || Object.keys(rawProfiles).length > MAX_PROFILES)
    ...
    for (const [name, value] of Object.entries(rawProfiles)) {
        ...
        if (!allow || allow.length === 0)
            throw new Error("invalid allow patterns");
        const allowProtected = item.allow_protected === undefined ? false : item.allow_protected;
        if (typeof allowProtected !== "boolean")
            throw new Error("invalid allow_protected");
        (0, glob_1.compileGlobs)(allow);
    ...
    if (!protectedPatterns)
        throw new Error("invalid protected patterns");
    (0, glob_1.compileGlobs)(protectedPatterns);
    ...
    if (noProfile !== "review" && noProfile !== "pass")
        throw new Error("invalid no_profile mode");
...
function selectProfile(policy, labels, explicit) {
    if (explicit !== undefined) {
        if (explicit === "")
            return { migration: policy.noProfile === "pass", reason: policy.noProfile === "pass" ? undefined : "NO_PROFILE" };
        if (!policy.profiles[explicit])
            return { migration: false, reason: "UNKNOWN_SCOPE_LABEL" };
    ...
    if (names.some((name) => !policy.profiles[name]))
        return { migration: false, reason: "UNKNOWN_SCOPE_LABEL" };
    if (names.length !== 1)
        return { migration: false, reason: "MULTIPLE_PROFILES" };
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.REASON_ORDER = void 0;
exports.sortReasons = sortReasons;
exports.REASON_ORDER = [
...
function sortReasons(codes) {
    const set = new Set(codes);
    return exports.REASON_ORDER.filter((code) => set.has(code));
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const main_1 = __nccwpck_require__(2957);
(0, main_1.runCli)(process.argv.slice(2)).then((code) => {
